...
%%
%%
%%     This code takes extensive advantage of several Erlang modules which ship with the Erlang/OTP
...
%%
%%     Stylistically we maintain an "single-assignment" functional style even when dealing with handles to data structure
%%     which could be modified opaquely.
%% 
%%     For example, because the "digraph" module introduces modifiable global data structures, the code snippet:
%%
%%          G = digraph:new(),
%%          process(G),
%%
%%     would imply that after "process(G)", G refers to a modified data structure which we 
%%     COULD then pass to another function:
%%
%%          process2(G),
%%          
%%    However, in this code we CHOOSE to maintain the single-assignment style to be consistent with the
%%    single-assignement handling of native data structures in Erlang.
%%    
%%    Thereofre, we CHOOSE to write the above in a single-assigment style this way:
%%
%%          G = digraph:new(),
%%          G2 = process(G),
%%          G3 = process2(G2),
%%  
%%    This keeps the code a little more syntaxically consistent with with the single-assigment 
%%    behavior of native Erlang data structures like simple variables and lists.  We will note
%%    where we intentionally deviate from this approach.
...
%%
%% Output: The Network Complexit Index (NCI) for the input graph as described in
%%         Bailey, Grossman. A Network Complexity Index for Networks of Networks. 2013
...
%% Examples:  [ {a,b},{a,c},{a,d},{a,d} ] would represent a graph with four vertices: a, b, c, d 
%%            and three edges: one edge between a and b, one betwen a and c, and one between a and d.
%%
%% Notes: Sizes of up to a million edges should run fast on modern laptop hardware.
...
compute(EdgeList)->
    ...
    %%  At this point G2 is a handle to an Erlang data structure
    %%  which represents an Erlang digraph which was expressed in the passed EdgeList
    %%  parameter including duplicate edges and directional edges
    ...
    %%  "for every digraph vertex Vertex in the list V, call clean_vertex(G2,Vertex)"
    %%
    ...
    %%  data structure. 
    %% 
    %%  At this point in the code G2 now points to a preprocessed digraph on which we want to
...
%% === calc_nci function is an internal function called by the externally callable compute(G) function === %%
%%
%% Input: an Erlang digraph represpensation of a graph in which every vertex has a 
%%        label representating community membership.  
%%
%%        The "prop_label" function below is one implementation of community detection
%%        which outputs a properly labeled graph
%%        
%%
...
calc_nci(LabeledGraph)->
    ...
    %% At this point in the code C refers to an unordered of community names with associated sizes
    %% For example if C = [{a,10},{f,3},{c,55}], it is a representation of three communities a, f, and c
    %% which have 10, 3, and 55 members respectively
    
    SC = rev_sort_labels(C),

    %% At this point, SC referes to a sorted list of communities with sizes which have been sorted by size
...
%%
%% === calc_nci(SortedList,NCI) is a recursive internal function called by the non-recursive calc_nci(LablelGraph) function above === %%
%%
%% This is a simple algorithms which just recursively walks a sorted list of {community,size} elements and returns the balance point between sizes and 
%% and numbers of the communities.
%%
%% For example the input list of  [{b,100},{c,100},{a,50},{z,43},{f,20},{g,3},{x,3},{p,2}] would return 5: "There are exactly 5 communties that are 
%% greater than or equal to 5 in size." 
%%
%% Input: A sorted list of communities with sizes which have been sorted by size
%%        such that the largest communties are at the head of the list. See example above.
%%
%%
%% Output: The Network Complexit Index for the input graph described in
%%         Bailey, Grossman. A Network Complexity Index for Networks of Networks. 2013
%%         http://www.flowforwarding.org/docs/Bailey%20-%20Grossman%20article%20on%20network%20complexity.pdf
%%
-spec calc_nci( SorteList :: list(), NCI :: integer() ) -> NCI :: integer().
calc_nci([],NCI)->
    NCI;
...
%% === get_communties(G) function is an internal function called by the calc_nci(LabeledGraph) function === %%
%%
%% Input: an Erlang digraph represpensation of a graph in which every vertex has a 
%%        label representating community membership.  
%%
...
%%        For example an output of [{a,10},{f,3},{c,55}] is a representation of three communities a, f, and c
%%        which have 10, 3, and 55 members respectively 
%%
%%
get_communities(G)->
...
%% WARNING: Recursive function
%%
%% === prop_labels(G) function is an internal function called by the externally callable compute(G) function === %
%%
%% This function implements exactly the label propagation algorithm detailed in:
%%
%% Ragahavan, Albert, Kumara. Near linear time algorithm to detect community structures in large-scale networks.
...
%%        
%%
%% Output: an Erlang digraph represpensation of a graph in which every vertex has a 
%%        label representating community membership.
%%
%% NOTES: This is a recursive function which has stop condition that no labels where changed on the preveious iteration.
%%        As detailed in the paper above, infinite oscillations are avoided by uniformaly randomly choosing ties for labelling,
%%        and randomly choosing vertex processing order on every iteration.
%% 
prop_labels(G)->

    %% This first section of code randomizes the vertex procesing order for each iteration.
    ...
    %% The processing function is applied to each member (left to right) of the list and results are 
    %% accumulated in the accumlator and returned at the end of the iteraion.

    %% Processing function is applied to every vertex is
    %% fun(Vertex,Acc)->
    %%       {StopCount,GoCount} = Acc,  % <-- this line EXTRACTS the current StopCount and GoCount from Acc
    ...
    %%           end
    %% 
    %%  This can be read as: attempt to label every vertex (see the label_vertex function)
    %%                       if the vertex got relabeled, Result will be "go".  
    %%                       if the label did not get relabeled, Result will be "stop"
    %%                       depeding on Result, increment either the GoCount or the StopCount
    %%  The accumulated {StopCount,GoCount} results will returned to the caller of foldl()  
    %%  after all the vertices have been processed
    %%
    %%  The initial conditions of the accumulator is passed to foldl(...) as {0,0,G} where 
    %%  G is a hangle to the digraph structure.  Passing G into the accumulator is not
    %%  necessary, we are doing to support the stylistic G2 single-assignment notation later
    %%  to aid in clarity.
    %% 
    %%  The list to run foldl(...) on is passes as V which by this point in the code is a 
    %%  randomized list of vertices of the Erlang graph passed into prop_labels(...)
    %% 
    ...
    %%     NewGoCount represents how many vertices were relabled in this iteration

    %% The following conditional ensures that we do another iteration if 
    %% at least one vertex was relabeled.
    %%
    %% The stop condition, as presented in the paper above, is that EVERY vertex is labeled identically to a majority
    %% of its neighbours.
    %% 
    %% The stop condition is represente as NewGoCount <= 0 == false and we then return the handle to the newly labeled graph
    %% whose handle is G3.
    %%
...
max_count(ListLabelCount)->
    lists:foldl(fun({_,CCount},MCount)->case CCount > MCount of
					    true -> CCount;
						       false -> MCount
					end
...
clean_neighbours(_G,_V,[],_P,Acc)->
    Acc;
clean_neighbours(G,V,SortedNL,P,Acc) ->
    [H|Rest] = SortedNL,
...
remove_dirt(G,V,Dirt) ->
    ...
    if 	
	V /= N -> 
	    remove_edge(G,N,V,[ digraph:edge(G,E) || E <- digraph:edges(G,V) ]);
	true -> ok
    end,
